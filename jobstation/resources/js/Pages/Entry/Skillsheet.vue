...
const form = useForm({
    basicInfo: {
        initial: props.temporaryData?.basicInfo?.initial ?? props.user?.initial ?? '',
        age: props.temporaryData?.basicInfo?.age ?? props.user?.age ?? 0,
        years_of_experience: props.temporaryData?.basicInfo?.years_of_experience ?? props.user?.years_of_experience ?? 0,
        sex: props.temporaryData?.basicInfo?.sex ?? props.user?.sex ?? '',
        is_japanese: props.temporaryData?.basicInfo?.is_japanese ?? props.user?.is_japanese ?? true,
        nationality: props.temporaryData?.basicInfo?.nationality ?? props.user?.nationality ?? '',
        education: props.temporaryData?.basicInfo?.education ?? props.user?.education ?? '',
        address: props.temporaryData?.basicInfo?.address ?? props.user?.address ?? '',
        train_line: props.temporaryData?.basicInfo?.train_line ?? props.user?.train_line ?? '',
        station: props.temporaryData?.basicInfo?.station ?? props.user?.station ?? '',
        qualifications: (props.temporaryData?.basicInfo?.qualifications ?? props.user?.qualifications ?? []).concat(Array(5).fill('')).slice(0, 5),
    },

    preferences: props.temporaryData?.preferences ?? (() => {
        const getBitNumber = (value) => {
            return value ? Math.log2(value) + 1 : 0;
        }
        return {
            desired_area: getBitNumber(props.user?.desired_area ?? 0),
            desired_start: getBitNumber(props.user?.desired_start ?? 0),
            desired_price: getBitNumber(props.user?.desired_price ?? 0),
            desired_remote: getBitNumber(props.user?.desired_remote ?? 0),
        }
    })(),

    experiences: props.temporaryData?.experiences ?? (() => {
        const experiences = props.user?.work_experiences?.length ? props.user.work_experiences : [{ ...experienceForm }];
        experiences.push(...Array.from({ length: Math.max(0, 3 - experiences.length) }, () => ({ ...experienceForm })));
        return experiences;
    })(),

    skill_score: props.temporaryData?.skill_score ?? (() => {
        ...
        for (const skillGroup in props.constants.skill_group) {
            ...
            if (Array.isArray(skills)) {
                ...
                for (const skill in skills) {
                    skillScoreData[skillGroup][skills[skill].name] = props.user?.skill_score?.[skillGroup]?.[skills[skill].name] ?? 0;
                }
            }
        }
    ...
    personality: props.temporaryData?.personality ?? (() => {
...
const temporarySave = () => {
    axios.post(route('entry.temporary-save'), {
        ...
        .catch(error => {
            window.alert('一時保存に失敗しました。');
        })
}

const limitInitialInput = (event, numCharacter) => {
    let value = event.target.value
    value = value.replace(/[^a-zA-Z・.]/g, '');
    value = value.slice(0, numCharacter);
    event.target.value = value;
}
const limitNumberInput = (event, numDigit) => {
    let value = event.target.value
        ...
        <template #main>
            <div class="py-12">
                <div class="max-w-7xl mx-auto px-6">
                    <form @submit.prevent="handleSubmitClick" class="bg-white p-6 shadow rounded-lg">
                        <div
                            class="sticky top-[85vh] sm:top-[20vh] flex justify-center sm:justify-end gap-4 text-center">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-2 gap-6">
                            <div class="col-span-2">
                                ...
                                <TextInput v-model="form.basicInfo.initial" type="text" class="mt-1 block w-full"
                                    required @input="limitInitialInput($event, 5)" placeholder="Y・T" />
                            </div>
                            <div>
                            ...
                            <div class="col-span-2">
                                ...
                                <div class="mt-1 space-x-4">
                                    <label class="inline-flex items-center">
                                        <input type="radio" v-model="form.basicInfo.sex" value="man" name="sex" required
                                            class="mr-2">
                                        男性
                                    </label>
                                    <label class="inline-flex items-center">
                                        <input type="radio" v-model="form.basicInfo.sex" value="woman" name="sex"
                                            class="mr-2">
                                        女性
                                    </label>
                                </div>
                            ...
                            <div class="col-span-2 sm:col-span-1">
                                ...
                                <div class="mt-1 space-x-4">
                                    <label class="inline-flex items-center">
                                        <input type="radio" v-model="form.basicInfo.is_japanese" :value="true"
                                            name="is_japanese" required class="mr-2">
                                        日本
                                    </label>
                                    <label class="inline-flex items-center">
                                        <input type="radio" v-model="form.basicInfo.is_japanese" :value="false"
                                            name="is_japanese" class="mr-2">
                                        その他
                                    </label>
                                </div>
                                <TextInput v-if="!form.basicInfo.is_japanese" v-model="form.basicInfo.nationality"
                                    type="text" class="mt-6 block w-full" :required="!form.basicInfo.is_japanese"
                                    maxlength="40" placeholder="アメリカ合衆国" />
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <InputLabel value="最終学歴" />
                                <TextInput v-model="form.basicInfo.education" type="text" class="mt-1 block w-full"
                                    maxlength="40" placeholder="令和大学　情報処理学部" />
                            </div>
                            <div class="col-span-2">
                            ...
                            <div class="col-span-2 flex flex-col sm:flex-row gap-4 w-full">
                                ...
                                <div class="flex items-center gap-4 w-full">
                                    ...
                                    <TextInput v-model="form.basicInfo.station" type="text" class="w-4/5" required
                                        maxlength="15" placeholder="錦糸町" />駅
                                </div>
                            </div>
                            <div class="col-span-2">
                                <InputLabel value="資格" />
                                <div v-for="(item, index) in 5" :key="index" class="flex items-center gap-4">
                                    {{ index + 1 }}
                                    <TextInput v-model="form.basicInfo.qualifications[index]"
                                        :modelValue="form.basicInfo.qualifications[index] ?? ''" type="text"
                                        class="mt-1 block w-full" maxlength="30"
                                        :placeholder="index === 0 ? 'ITパスポート' : (index === 1 ? '基本情報技術者試験' : '')" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <SelectInput v-model="form.preferences.desired_price" class="mt-1 block w-full">
                                    <option v-for="(price, key) in constants?.check_items.price" :key="key"
                                        :value="price.bit_number">
                                        {{ price.display_name }}
                                    </option>
                                </SelectInput>
                            </div>
                            <div>
                                ...
                                <SelectInput v-model="form.preferences.desired_start" class="mt-1 block w-full">
                                    <option v-for="(month, key) in constants?.check_items.start" :key="key"
                                        :value="month.bit_number">
                                        {{ month.display_name }}
                                    </option>
                                </SelectInput>
                            </div>
                            <div>
                                ...
                                <SelectInput v-model="form.preferences.desired_remote" class="mt-1 block w-full">
                                    <option v-for="(remote, key) in constants?.check_items.remote" :key="key"
                                        :value="remote.bit_number">
                                        {{ remote.display_name }}
                                    </option>
                                </SelectInput>
                            </div>
                            <div>
                                ...
                                <SelectInput v-model="form.preferences.desired_area" class="mt-1 block w-full">
                                    <option v-for="(area, key) in constants?.check_items.area" :key="key"
                                        :value="area.bit_number">
                                        {{ area.display_name }}
                                    </option>
                                </SelectInput>
                            </div>
                        ...
                        <div class="mt-8">
                            <h2 class="text-lg font-medium mb-6">業務経歴 <span
                                    class="text-blue-500">※直近、13経歴まで最大追加可能</span></h2>
                                ...
                                <div class="grid grid-cols-2 gap-6">
                                    <div class="col-span-2">
                                        ...
                                        <div class="flex items-center gap-4">
                                            <TextInput v-model="experience.period" type="number"
                                                class="mt-1 block w-1/2" :required="!index"
                                                @input="limitNumberInput($event, 3)" :placeholder="8" />
                                    ...
                                    <div>
                                        ...
                                        <SelectInput v-model="experience.pj_scale_id" class="mt-1 block w-full"
                                            :required="!index">
                                            <option v-for="(pj_scale, key) in constants?.pj_scale" :key="key"
                                                :value="pj_scale.id">
                                    ...
                                    <div>
                                        ...
                                        <TextInput v-model="experience.team_size" type="number"
                                            class="mt-1 block w-full" :required="!index"
                                            @input="limitNumberInput($event, 3)" :placeholder="28" />
                                    </div>
                                    ...
                                    <div class="col-span-2">
                                        ...
                                        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 mt-2">
                                            <div v-for="(display_name, name) in props.constants.work_experience.work"
                                                class="flex gap-2" :key="name">
                                                <Checkbox v-model="experience[name]" :checked="experience[name]">
                                                </Checkbox>
                                    ...
                                    <div class="col-span-2">
                                        ...
                                        <textarea v-model="experience.description"
                                            ...
                                            rows="3" :required="!index" maxlength="500"
                                            placeholder="■総合管理システム開発支援・社員用Webページの作成"></textarea>
                                    </div>
                                </div>
                                <div class="grid grid-cols-1 gap-4 mt-4">
                                    <div v-for="(display, name) in props.constants.work_experience.tech">
                                        <InputLabel :value="display.display_name" />
                                        <TextInput v-model="experience[name]" class="mt-1 block w-full" maxlength="255"
                        ...
                        <div class="mt-8">
                            ...
                            <h2 class="flex flex-wrap text-lg justify-center gap-x-6">
                                <span>1：独学</span><span>2：指示が必要</span><span>3：確認が必要</span><span>4：一人称でできる</span><span>5：指導できる</span>
                            </h2>
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                <div class="mt-10" v-for="(displayName, name) in props.constants.skill_group"
                                    :key="name">
                                    <div class="flex items-center justify-center gap-4 mb-2">
                                        <span class="text-lg font-semibold min-w-[135px]">{{ displayName }}</span>
                                        <span class="text-center w-24">自己評価</span>
                                    </div>
                                    <div v-for="(item, itemIndex) in props.constants.check_items[name]"
                                        :key="itemIndex">
                                        <div class="flex items-center justify-center gap-4 mt-6">
                                            <span class="min-w-[135px]">{{ item.display_name }}</span>
                                            <SelectInput class="w-24" v-model="form.skill_score[name][item.name]">
                                                <option v-for="n in constants.ratings" :key="n" :value="n">{{ n }}
                                                </option>
                                            </SelectInput>
                                        </div>
                                    </div>
                        ...
                        <div class="mt-8 mb-20">
                            ...
                            <h2 class="flex flex-wrap text-lg justify-center gap-x-6">
                                <span>1：できない</span><span>2：少しできる</span><span>3：普通</span><span>4：できる</span><span>5：良くできる</span>
                            </h2>
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            <Modal :show="showConfirmModal" @close="showConfirmModal = false">
                <div class="p-6">
                    ...
                    <div class="mt-4">
                        <p class="text-sm text-gray-600">スキルシートを登録してエントリーを行います。</p>
